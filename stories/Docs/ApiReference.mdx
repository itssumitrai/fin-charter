...
Adjusts bar spacing so all loaded bars fit within the chart width. This is called automatically on the first paint after data is loaded, so charts fill the viewport by default with no gaps on left or right.
...
Composites all panes into a single canvas and returns it. The canvas is pre-filled with the chart's `layout.backgroundColor`, so the screenshot matches the live chart appearance (including dark themes).
...
interface LayoutOptions {
  ...
  textColor:       string;  // default: '#d1d4dc'
